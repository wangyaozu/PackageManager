...
- 看板统计功能：
  - 新增 KanbanStatsPage 页面，支持项目、人员、迭代筛选
...
- 包管理筛选的优化：
  - 实现可见行版本信息加载功能
  - 新增 LoadVersionsWhichVisiableFromFtpAsync 方法用于加载可见包版本
  - 修改主窗口初始化逻辑，替换原有的全量加载为可见行加载
  - 更新刷新按钮点击事件，优先加载可见行版本信息
  - 在窗口内容渲染完成后触发一次可见行版本加载

- 包属性变更事件优化：
  - 恢复 PackageInfo 属性变化监听中的状态文本更新逻辑
...
- 包属性变更事件优化：
  - 注释掉包属性变更事件中的状态更新逻辑
  - 暂时禁用 LatestActivePackage 的自动更新机制
  - 为后续重构保留代码参考
